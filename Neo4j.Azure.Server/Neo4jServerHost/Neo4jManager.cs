    ...
    public class Neo4jManager
    {
        ...
        internal void DownloadNeo4j(IPaths paths, CloudStorageAccount storageAccount)
        {
            try
            {
                ...
                var blob = blobClient.GetBlobReferenceFromServer(
                    new Uri(neo4jBlobAddress)
                    );

                var option = new BlobRequestOptions();
                option.MaximumExecutionTime = new TimeSpan(0, 15, 0);
                using (var fileStream = File.OpenWrite(localNeo4JZipPath))
                {
                    blob.DownloadToStream(fileStream, options: option);
                }
        ...
        public void DownloadJava(IPaths paths, CloudStorageAccount storageAccount)
        {
            try
            {
                ...
                var blob = blobClient.GetBlobReferenceFromServer(
                    new Uri(javaBlobAddress)
                    );

                var option = new BlobRequestOptions();
                option.MaximumExecutionTime = new TimeSpan(0, 15, 0);
                
                using (var fileStream = File.OpenWrite(localJavaZipPath))
                {
                    blob.DownloadToStream(fileStream, options: option);
                }
